...
export const pendingSlice = createSlice({
	...
	reducers: {
		...
		addPendings: (state, action) => { 
			...
			return {
				...state,
				activeCount: status === STATUS.ACTIVE ? (state.activeCount + 1) : state.activeCount,
				doneCount: status === STATUS.DONE ? (state.doneCount + 1) : state.doneCount,
...
export const { addPendings, setPendings, changeStatusPending } = pendingSlice.actions;
